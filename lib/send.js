...
class Send {
  ...
  createTransaction (outputs, walletInfo, utxos) {
    try {
      ...
      // const signedTx = unsignedTx.sign(xkeyChain).toString()

      // Sign the tx.
      const signedBuff = unsignedTx.sign(xkeyChain).toBuffer()

      // Convert to a hex string.
      const signedHex = `0x${this.bintools.addChecksum(signedBuff).toString('hex')}`
