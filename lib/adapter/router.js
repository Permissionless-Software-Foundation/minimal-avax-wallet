...
class AdapterRouter {
  ...
  async getUTXOs (address) {
    try {
      ...
      if (this.interface === 'rest-api') {
        const { utxos: utxoSet } = await this.ava.XChain().getUTXOs(address)
  ...
  async getBalance (address) {
    try {
      ...
      if (this.interface === 'rest-api') {
        return await this.ava.XChain().getAllBalances(address)
      }
  ...
  async getAssetDescription (assetId) {
    try {
      ...
      if (this.interface === 'rest-api') {
        const assetDetails = await this.ava.XChain().getAssetDescription(assetId)
  ...
  async issueTx (baseTx) {
    try {
      ...
      if (this.interface === 'rest-api') {
        const txid = await this.ava.XChain().issueTx(baseTx)
