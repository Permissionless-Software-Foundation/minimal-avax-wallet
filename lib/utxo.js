...
class UTXOs {
  ...
  async getUTXOs (addr) {
    ...
    const { utxos: utxoSet } = await this.ava.XChain().getUTXOs(addr)
    const utxos = utxoSet.getAllUTXOs().map(_this.decodeUtxo)
  ...
  async getBalance (addr) {
    ...
    const balances = await this.ava.XChain().getAllBalances(addr)

    if (balances.length === 0) {
      return []
    }

    const promises = balances.map(asset => this.xchain.getAssetDescription(asset.asset))
  ...
  encodeUtxo (utxoJSON, address) {
    ...
    const utxoObject = new avm.TransferableInput(
  ...
  decodeUtxo (utxoObject) {
    ...
    const utxoJSON = {
      txid: this.bintools.cb58Encode(txid),
